...
Releases are created automatically when a PR merges to `main`. The `compute-version` job reads the `semver:` labels of every merged PR whose merge commit falls in the release window and bumps the version accordingly. The window is computed with `git rev-list "<latest-tag>..HEAD"`: the commits reachable from the pushed commit (the one `auto-release` will tag) but not from the previous release tag. Deciding membership by commit reachability rather than a merge-timestamp comparison fixes two bugs (see issue #128): the just-released PR is no longer re-counted (GitHub records `mergedAt` a moment after the merge commit's committer date), and a PR that merges while an earlier run is still in flight is no longer counted in both that run and its own. The `mergedAt`-versus-tag-date comparison remains only as a fallback for a PR with no recorded merge commit. No manual tagging is required for day-to-day releases.
